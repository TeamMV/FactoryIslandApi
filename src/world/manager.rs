use crate::event::world::BeforeChunkGenerateEvent;
...
impl ChunkManager {
    pub fn try_load_chunk(&self, world: &World, chunk_pos: ChunkPos, event_bus: &mut EventBus<Event>) -> Option<Chunk> {
        ...
        match file {
            Ok(mut file) => {
                ...
                if let Ok(decompressed) = decompress_size_prepended(compressed.as_slice()) {
                    ...
                    let mut chunk = chunk.generate_terrain(world.generator(), event_bus, world.objects());
